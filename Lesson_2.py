...
def add_student(student: dict) -> dict | None:
    ...
    if not student.get("name") or not student.get("marks"):
        ...
        # action
        student ["id"] = int(next(iter(storage[-1].values()))) + 1
        student ["info"] = str()
        student ["marks"] = list()
        print(student["id"])
...
def student_management_command_handle(command: str):
    ...
    elif command == "student id":
        student_id: str = int(input("Enter Student ID: "))
        search_student(student_id=int(student_id))
...
def handle_user_input():
    ...
    STUDENT_MANAGEMENT_COMMANDS = ("show", "add", "search","student id")
